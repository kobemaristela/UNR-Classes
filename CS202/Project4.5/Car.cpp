...
void Car::print(){
    ...
    //Prints Sensors
    for (int x = 0; x < 3; x++) {
        if ((myStringCompare(ptrSensor->getM_type(), "none")))
        {
            cout << ptrSensor++->getM_type();
            if(myStringCompare((ptrSensor+1)->getM_type(), "none")){
                cout << ", ";
            }
        }
    }
...
Car& Car::operator+(Sensor &rhs) {
    ...
    for (int x = 0; x < 3; x++, ptrSensor++) {
        if (!(myStringCompare(ptrSensor->getM_type(), "none"))) {
            *ptrSensor = rhs;
            //cout << "Operator: " << ptrSensor->getM_type() << endl;
            break;
        }
    }
    return *this;
